...
var parseBookmarks = function parseBookmarks(rows) {
	...
	if (fails.length > 0) {
		log.global.error({ fails: JSON.stringify(fails) }, 'failed to reparse ' + fails.length + ' bookmarks.');
	}

	return Ok(oks).productOf();
};

var sendBookmarks = function sendBookmarks(res, maxID, rowResult) {

	rowResult.onErr(function (err) {

		log.global.error({
			err: err, resId: res.id, maxID: maxID
	...
	rowResult.onOk(function (rows) {
		...
		Ok(rows).then(parseBookmarks).then(function (bookmarks) {
			...
			var body = JSON.stringify({ data: bookmarks, nextID: nextID });
...
var fetchBookmarks = function fetchBookmarks(maxID, amount, res) {
