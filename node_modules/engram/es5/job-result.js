#!/usr/bin/env node


"use strict";

var is = require('is');
var log = require('engram/es5/logging/logger');

var constants = require('engram/es5/constants');
var message = require('engram/es5/message');

constants.BUS.on(message.TITLE_FINISHED, function (saveResult) {

	saveResult.onErr(function (err) {
		log.jobResult.error({ err: err }, 'title extraction failed.');
	});

	saveResult.onOk(function (url) {

		if (!is.undefined(url)) {
			log.jobResult.debug('successfully extracted title for ' + url);
		}
	});
});

constants.BUS.on(message.SOFTFAIL_FINISHED, function (saveResult) {

	saveResult.onErr(function (err) {
		log.jobResult.error({ err: err }, 'soft-fail determination failed.');
	});

	saveResult.onOk(function (url) {

		if (!is.undefined(url)) {
			log.jobResult.debug('successfully determinated soft-fail status for ' + url);
		}
	});
});

constants.BUS.on(message.STATUS_FINISHED, function (saveResult) {

	saveResult.onErr(function (err) {
		log.jobResult.error({ err: err }, 'status determination failed.');
	});

	saveResult.onOk(function (url) {

		if (!is.undefined(url)) {
			log.jobResult.debug('successfully determinated fail status for ' + url);
		}
	});
});

constants.BUS.on(message.ARCHIVE_FINISHED, function (archiveResult) {

	archiveResult.onErr(function (err) {
		log.jobResult.error({ err: err }, 'status determination failed.');
	});

	archiveResult.onOk(function (url) {

		if (!is.undefined(url)) {
			log.jobResult.debug('successfully saved archive for ' + url);
		}
	});
});
