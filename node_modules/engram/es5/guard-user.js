...

{
	...
	(function () {
		...
		var whitelist = fs.readFileSync('data/ip-whitelist').toString().split('\n').filter(function (val) {
			return val && val.length > 0;
		});

		logIpAddress = function logIpAddress(req) {

			utils.getIpAddress(req).onOk(function (ip) {

				var isUnknown = whitelist.indexOf(ip) === -1;

				isUnknown ? log.security.warning({
					ip: ip,
					url: req.originalURL
				}, 'non-whitelisted ip address connected!') : log.security.info({
					ip: ip,
					url: req.originalURL
