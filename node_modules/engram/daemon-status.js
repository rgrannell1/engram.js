...
var saveStatus = function (recall, state, urls) {

	saveStatus.precond(recall, state, urls);

	var onURL = function (urlsResult) {

		onURL.precond(urlsResult);

		urlsResult.onErr(function (errData) {
			recall(Err(errData), state.control.delay);
		});

		urlsResult.onOk(function (_ref) {
			var res = _ref.res;
			var body = _ref.body;

			updateStatus(urls, res.statusCode, function (result) {
				recall(Ok(urls), state.control.delay);
			});
		});
	};
	...
	urls.length === 0 ? recall(Ok([]), state.control.delay) : requestURL(urls[0].url, {}, onURL);
};

saveStatus.precond = function (recall, state, urls) {
