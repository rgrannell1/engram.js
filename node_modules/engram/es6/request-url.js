...

{
	...
	var freshRequest = (uri, options, callback, fresh) => {
		...
		if (runningCount >= constants.REQUEST_COUNT) {
			...
			++runningCount
			log.global.trace({fresh, runningCount}, `GET ${uri} (cache miss)`)

			var requestStart = process.hrtime( )

			request(uri, options, (err, res, body) => {

				constants.BUS.emit( message.REQUEST_ENDED, {runningCount} )
				--runningCount
				...
				if (err) {
					...
					constants.BUS.emit( message.URL_RESOLVED, Err({uri, err}) )
					...
					constants.BUS.emit( message.URL_RESOLVED, Ok({res, body}) )
...
var cachedRequest = (uri, options, callback, fresh = false) =>{
	...
	if (is.undefined(pending)) {
		...
		var isTimely   = difference < constants.HTTP_TIMEOUT
