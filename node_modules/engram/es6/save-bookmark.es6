...
var insertURL = (url, time, callback) => {
	...
	database.do((db, callback) => {
		...
		db.run('INSERT  OR IGNORE INTO url VALUES (NULL, $url, $ctime, $mtime);', {
...
var insertBookmark = (url, time, callback) => {
	...
	insertURL(url, time, function (db, result) {

		if (result.isErr) {
			callback(result)
		}

		db.run('INSERT INTO bookmark VALUES (NULL, (SELECT url_id FROM url WHERE url.url = $url), $ctime, $mtime);', {
