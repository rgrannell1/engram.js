...
var isSoftFailer = (url, callback) => {
	// -- TODO

	requestURL(url, { }, reqResult => {
...
var saveSoftFailStatus = (recall, state, urls) => {

	urls.length === 0
		? recall(Ok(undefined), state.control.delay)
		: isSoftFailer(urls[0].url, statusResult => {
			...
			statusResult.onOk(isSoftFailer => {
				...
				updateSoftFail(urls[0].url, isSoftFailer, updateResult => {
