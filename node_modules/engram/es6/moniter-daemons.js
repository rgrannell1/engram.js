...
var daemon        = require('engram/daemon')
...
var startJob = (jobName, job, registry) => {
	...
	if (neverStarted) {
		...
		daemon.start(job.moniter, daemon.wrapReporter(reportAlive.bind({ }, registry), job.reporter), job.state)
		reportAlive(registry)
...
var moniterWorkers = jobs => {
	...
	daemon.start(moniterManager, moniterReporter, moniterState)
