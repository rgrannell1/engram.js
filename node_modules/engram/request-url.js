	...
	(function () {
		...
		freshRequest = function (uri, options, callback, fresh) {
			...
			if (runningCount >= constants.REQUESTCOUNT) {
				...
				++runningCount;
				log.info({ fresh: fresh, runningCount: runningCount }, "GET " + uri + " (cache miss)");
				...
				request(uri, options, function (err, res, body) {
					...
					if (err) {
						...
						constants.BUS.fire(":resolved", Err({ uri: uri, err: err }));

						update(uri, { res: res, body: body });
						callback(Err({ uri: uri, err: err }));
					} else {

						constants.BUS.fire(":resolved", Err({ res: res, body: body }));
