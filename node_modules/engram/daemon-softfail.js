...
var isSoftFailer = function (url, callback) {
	// -- TODO

	requestURL(url, {}, function (reqResult) {
...
var saveSoftFailStatus = function (recall, state, urls) {

	urls.length === 0 ? recall(Ok(undefined), state.control.delay) : isSoftFailer(urls[0].url, function (statusResult) {
		...
		statusResult.onOk(function (isSoftFailer) {
			...
			updateSoftFail(urls[0].url, isSoftFailer, function (updateResult) {
				recall(updateResult, state);
			});
		});
	});
};
...
var moniter = function (recall, state) {
	findUncheckedURL(function (urlResult) {
		...
		urlResult.then(saveSoftFailStatus.bind({}, recall, state));
	});
};

var reporter = function (args, state) {
	return state;
};
