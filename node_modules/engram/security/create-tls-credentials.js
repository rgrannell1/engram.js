...
callbacks.writeResultOk = callback => {
	return ( ) => {
		...
		checkTLSCredentials({keypath, certpath}, exists => {
			...
			if (!exists) {
				...
				log.security.error({keypath, certpath}, 'created tls certificates.')
				callback(Ok( ))
...
callbacks.writeResultErr = callback => {
	return err => {
...
var createTLSCredentials = (keypath, certpath, callback) => {
	...
	pem.createCertificate(credentialConfig, (err, keys) => {
		...
		writeResult.onOK( callbacks.writeResultOk(callback))
		writeResult.onErr(callbacks.writeResultErr(callback))
